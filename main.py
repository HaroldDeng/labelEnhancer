...
if __name__ == '__main__':
    ...
    px = Image.open('imgs/Image_20200613150453211.jpg').convert('RGBA')
    jData = json.loads(open('imgs/Image_20200613150453211.json', 'rb').read())
    mk = jData['shapes'][0]
    mk['param'] = numpy.array(mk.pop('points'))
    mk['type'] = mk.pop('shape_type')


    cut_px, cut_mk = pixFac.copyRegion(px, mk)
    res_px, res_mk = pixFac.pasteRegion(px, [mk], cut_px, cut_mk, (700, 300))
    for i in range(9):
        i += 1
        res_px, res_mk = pixFac.pasteRegion(res_px, res_mk, cut_px, cut_mk, (700 + 50 * i, 300 + 50 * i))

    res_mk[0]['points'] = res_mk[0].pop('param').tolist()
    res_mk[0]['shape_type'] = res_mk[0].pop('type')
    i = 1
    while i < len(res_mk):
        res_mk[i]['points'] = res_mk[i].pop('param').tolist()
        res_mk[i]['shape_type'] = res_mk[i].pop('type')
        res_mk[i]['label'] = "WeiLong{}".format(i + 1)
        res_mk[i]['line_color'] = None
        res_mk[i]['fill_color'] = None
        res_mk[i]['flags'] = {}
        i += 1
    ...
    buf = io.BytesIO()
    res_px = res_px.convert('RGB')
    res_px.save(buf, format='JPEG')  # save as byte data
    jData['imageData'] = base64.b64encode(buf.getvalue()).decode('ascii')
    jData['imageHeight'] = res_px.size[1]
    jData['imageWidth'] = res_px.size[0]

    res_px.save('tmp/Image_20200613150453211.jpg')
    json.dump(jData, open('tmp/Image_20200613150453211.json', 'w+'), indent=2)
